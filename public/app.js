...
// Carregar histórico de operações críticas
const carregarHistoricoOperacoes = async () => {
    try {
        const response = await api('/admin/historico-operacoes');
        const container = document.getElementById('historicoOperacoes');

        if (response && response.historico && Array.isArray(response.historico)) {
            if (response.historico.length === 0) {
                container.innerHTML = '<p style="text-align: center; color: #6b7280; font-style: italic;">Nenhuma operação crítica registrada</p>';
                return;
            }
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: #f9fafb; border-bottom: 2px solid #e5e7eb;">
                                <th style="padding: 0.75rem; text-align: left; font-weight: 600; color: #374151;">Data/Hora</th>
                                <th style="padding: 0.75rem; text-align: left; font-weight: 600; color: #374151;">Operação</th>
                                <th style="padding: 0.75rem; text-align: left; font-weight: 600; color: #374151;">Usuário</th>
                                <th style="padding: 0.75rem; text-align: left; font-weight: 600; color: #374151;">Justificativa</th>
                                <th style="padding: 0.75rem; text-align: left; font-weight: 600; color: #374151;">Detalhes</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${response.historico.map((op, index) => `
                                <tr style="border-bottom: 1px solid #f3f4f6; ${index % 2 === 0 ? 'background: #fafafa;' : ''}">
                                    ...
                                    <td style="padding: 0.75rem;">
                                        <span style="background: ${op.tipo === 'DELETE_AIH' ? '#fef2f2' : '#fefce8'}; 
                                                     color: ${op.tipo === 'DELETE_AIH' ? '#dc2626' : '#d97706'}; 
                                                     padding: 0.25rem 0.75rem; border-radius: 9999px; font-size: 0.75rem; font-weight: 500;">
                                            ${op.tipo === 'DELETE_AIH' ? '💀 Exclusão AIH' : '🗑️ Exclusão Movimentação'}
                                        </span>
                                    </td>
                                    <td style="padding: 0.75rem; font-weight: 500; color: #374151;">${op.usuario}</td>
                                    <td style="padding: 0.75rem; max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${op.justificativa}">
                                        ${op.justificativa}
                                    </td>
                                    <td style="padding: 0.75rem; font-size: 0.875rem; color: #6b7280;">
                                        ${op.detalhes}
            ...
            container.innerHTML = '<p style="text-align: center; color: #ef4444;">Erro ao carregar histórico</p>';
        }
    } catch (err) {
...
let dadosExclusaoAtual = null;

// Event listeners para os formulários de exclusão
document.addEventListener('DOMContentLoaded', () => {
    // Formulário deletar movimentação
    const formDeletarMov = document.getElementById('formDeletarMovimentacao');
    if (formDeletarMov) {
        formDeletarMov.addEventListener('submit', async (e) => {
            e.preventDefault();
            
            const numeroAIH = document.getElementById('aihMovimentacao').value.trim();
            ...
            if (!numeroAIH || !movimentacaoId || !justificativa) {
        ...
        // Carregar movimentações quando AIH for informada
        const inputAIH = document.getElementById('aihMovimentacao');
        if (inputAIH) {
            inputAIH.addEventListener('blur', carregarMovimentacoesAIH);
        }
    }

    // Formulário deletar AIH completa
    const formDeletarAIH = document.getElementById('formDeletarAIH');
    if (formDeletarAIH) {
        formDeletarAIH.addEventListener('submit', async (e) => {
            e.preventDefault();
...
// Carregar movimentações de uma AIH específica
const carregarMovimentacoesAIH = async () => {
    const numeroAIH = document.getElementById('aihMovimentacao').value.trim();
    const selectMov = document.getElementById('movimentacaoId');

    if (!numeroAIH) {
        selectMov.innerHTML = '<option value="">Selecione uma movimentação</option>';
        return;
    }

    try {
        const response = await api(`/aih/${numeroAIH}/movimentacoes-detalhadas`);
        
        if (response && response.movimentacoes && Array.isArray(response.movimentacoes)) {
            selectMov.innerHTML = '<option value="">Selecione uma movimentação</option>';
            
            response.movimentacoes.forEach(mov => {
                const option = document.createElement('option');
                option.value = mov.id;
                option.textContent = `ID ${mov.id} - ${mov.tipo === 'entrada_sus' ? 'Entrada SUS' : 'Saída Hospital'} - ${new Date(mov.data_movimentacao).toLocaleDateString('pt-BR')} - Status ${mov.status_aih}`;
                selectMov.appendChild(option);
            });

            if (response.movimentacoes.length === 0) {
                selectMov.innerHTML = '<option value="">Nenhuma movimentação encontrada</option>';
            }
        } else {
            selectMov.innerHTML = '<option value="">AIH não encontrada</option>';
        }
    } catch (err) {
        console.error('Erro ao carregar movimentações:', err);
        selectMov.innerHTML = '<option value="">Erro ao carregar movimentações</option>';
    }
};

// Mostrar modal de confirmação
const mostrarModalConfirmacaoExclusao = () => {
    const modal = document.getElementById('modalConfirmacaoExclusao');
    const detalhes = document.getElementById('detalhesExclusao');

    if (dadosExclusaoAtual.tipo === 'movimentacao') {
        detalhes.innerHTML = `
            <p><strong>Operação:</strong> Exclusão de Movimentação</p>
            <p><strong>AIH:</strong> ${dadosExclusaoAtual.numeroAIH}</p>
            <p><strong>Movimentação ID:</strong> ${dadosExclusaoAtual.movimentacaoId}</p>
            <p><strong>Justificativa:</strong> ${dadosExclusaoAtual.justificativa}</p>
        `;
    } else {
        detalhes.innerHTML = `
            <p><strong>Operação:</strong> Exclusão de AIH Completa</p>
            <p><strong>AIH:</strong> ${dadosExclusaoAtual.numeroAIH}</p>
            <p><strong>Justificativa:</strong> ${dadosExclusaoAtual.justificativa}</p>
            <p style="color: #dc2626; font-weight: 600;">⚠️ Todos os dados relacionados serão excluídos permanentemente!</p>
        `;
    }

    document.getElementById('senhaAdminExclusao').value = '';
    modal.classList.add('ativo');
};
...
window.confirmarExclusao = async () => {
    ...
    if (!dadosExclusaoAtual) {
        alert('Dados de exclusão não encontrados');
        return;
    }
    ...
    try {
        ...
        if (dadosExclusaoAtual.tipo === 'movimentacao') {
            endpoint = '/admin/deletar-movimentacao';
            dados = {
                numero_aih: dadosExclusaoAtual.numeroAIH,
                movimentacao_id: parseInt(dadosExclusaoAtual.movimentacaoId),
                justificativa: dadosExclusaoAtual.justificativa,
                senha_admin: senhaAdmin
            };
        } else {
            endpoint = '/admin/deletar-aih';
            dados = {
                numero_aih: dadosExclusaoAtual.numeroAIH,
                justificativa: dadosExclusaoAtual.justificativa,
                senha_admin: senhaAdmin
        ...
        // Fechar modal e limpar formulários
        cancelarExclusao();
        
        // Limpar formulários
        if (dadosExclusaoAtual.tipo === 'movimentacao') {
            document.getElementById('formDeletarMovimentacao').reset();
            document.getElementById('movimentacaoId').innerHTML = '<option value="">Selecione uma movimentação</option>';
        } else {
