...
window.carregarLogsExclusao = async () => {
    ...
    container.innerHTML = `
        <div style="padding: 2rem; text-align: center;">
            ...
            <p style="color: #6366f1; font-weight: 500;">Carregando logs de exclusão...</p>
        </div>
    `;
    ...
    try {
        ...
        if (response && response.resultado && Array.isArray(response.resultado)) {
            ...
            if (logs.length === 0) {
                container.innerHTML = `
                    <div style="padding: 2rem; text-align: center;">
                        ...
                        <h4 style="color: #059669; margin: 0 0 0.5rem 0;">Nenhuma exclusão registrada</h4>
                        <p style="color: #64748b; margin: 0;">O sistema não possui registros de exclusões.</p>
                    </div>
                `;
            } else {
                // Cabeçalho da tabela
                let html = `
                    <div style="padding: 1rem; background: #f8fafc; border-bottom: 2px solid #e5e7eb; display: grid; grid-template-columns: 80px 120px 150px 1fr 120px; gap: 1rem; align-items: center; font-weight: 600; color: #374151; font-size: 0.875rem;">
                        <div>ID</div>
                        <div>Tipo</div>
                        <div>Usuário</div>
                ...
                logs.forEach((log, index) => {
                    const corTipo = log.tipo_exclusao === 'aih_completa' ? '#dc2626' : '#f59e0b';
                    const iconeTipo = log.tipo_exclusao === 'aih_completa' ? '💀' : '🗑️';
                    const descricaoTipo = log.tipo_exclusao === 'aih_completa' ? 'AIH Completa' : 'Movimentação';
                    
                    html += `
                        <div style="padding: 1rem; border-bottom: 1px solid #f3f4f6; display: grid; grid-template-columns: 80px 120px 150px 1fr 120px; gap: 1rem; align-items: center; transition: background-color 0.2s; cursor: pointer;" 
                             onmouseover="this.style.backgroundColor='#f8fafc'" 
                             onmouseout="this.style.backgroundColor='white'"
                             onclick="mostrarDetalhesLog(${JSON.stringify(log).replace(/"/g, '&quot;')})">
                            <div style="font-weight: 600; color: #6366f1;">#${log.id}</div>
                            <div style="display: flex; align-items: center; gap: 0.5rem;">
                                <span style="font-size: 1.2rem;">${iconeTipo}</span>
                                <span style="color: ${corTipo}; font-weight: 500; font-size: 0.875rem;">${descricaoTipo}</span>
                            </div>
                            <div style="color: #374151; font-weight: 500;">${log.usuario_nome || 'Sistema'}</div>
                            <div style="color: #64748b; font-size: 0.875rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${log.justificativa}">
                                ${log.justificativa ? log.justificativa.substring(0, 80) + (log.justificativa.length > 80 ? '...' : '') : 'Sem justificativa'}
                            </div>
                            <div style="color: #64748b; font-size: 0.875rem;">
                                ${new Date(log.data_exclusao).toLocaleString('pt-BR')}
                            </div>
                        </div>
                    `;
                });
                ...
                // Resumo no final
                const totalMovimentacoes = logs.filter(l => l.tipo_exclusao === 'movimentacao').length;
                const totalAIHs = logs.filter(l => l.tipo_exclusao === 'aih_completa').length;

                html += `
                    <div style="padding: 1rem; background: #f8fafc; border-top: 2px solid #e5e7eb; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
                        <div style="display: flex; gap: 2rem; flex-wrap: wrap;">
                            <div style="text-align: center;">
                                <div style="font-size: 1.5rem; font-weight: 700; color: #6366f1;">${logs.length}</div>
                                <div style="font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em;">Total</div>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 1.5rem; font-weight: 700; color: #f59e0b;">${totalMovimentacoes}</div>
                                <div style="font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em;">Movimentações</div>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 1.5rem; font-weight: 700; color: #dc2626;">${totalAIHs}</div>
                                <div style="font-size: 0.75rem; color: #64748b; text-transform: uppercase; letter-spacing: 0.05em;">AIHs Completas</div>
                            </div>
                        </div>
        ...
        container.innerHTML = `
            <div style="padding: 2rem; text-align: center;">
                <div style="font-size: 3rem; margin-bottom: 1rem; opacity: 0.7;">❌</div>
                <h4 style="color: #dc2626; margin: 0 0 0.5rem 0;">Erro ao carregar logs</h4>
                <p style="color: #64748b; margin: 0 0 1rem 0;">${err.message}</p>
                <button onclick="carregarLogsExclusao()" style="background: #6366f1; color: white; border: none; padding: 0.5rem 1rem; border-radius: 6px; cursor: pointer;">
                    🔄 Tentar novamente
                </button>
            </div>
        `;
...
window.mostrarDetalhesLog = (log) => {
    const modal = document.getElementById('modal');
    const modalContent = modal.querySelector('.modal-content');

    let dadosDetalhados = '';
    try {
        const dados = JSON.parse(log.dados_excluidos);
        if (log.tipo_exclusao === 'aih_completa') {
            dadosDetalhados = `
                <h5>📋 AIH Excluída:</h5>
                <ul>
                    <li><strong>Número:</strong> ${dados.aih?.numero_aih || 'N/A'}</li>
                    <li><strong>Valor Inicial:</strong> R$ ${(dados.aih?.valor_inicial || 0).toFixed(2)}</li>
                    <li><strong>Competência:</strong> ${dados.aih?.competencia || 'N/A'}</li>
                    <li><strong>Movimentações removidas:</strong> ${dados.totais?.movimentacoes || 0}</li>
                    <li><strong>Glosas removidas:</strong> ${dados.totais?.glosas || 0}</li>
                    <li><strong>Atendimentos removidos:</strong> ${dados.totais?.atendimentos || 0}</li>
                </ul>
            `;
        } else {
            dadosDetalhados = `
                <h5>🗑️ Movimentação Excluída:</h5>
                <ul>
                    <li><strong>AIH:</strong> ${dados.numero_aih || 'N/A'}</li>
                    <li><strong>Tipo:</strong> ${dados.tipo === 'entrada_sus' ? 'Entrada SUS' : 'Saída Hospital'}</li>
                    <li><strong>Valor:</strong> R$ ${(dados.valor_conta || 0).toFixed(2)}</li>
                    <li><strong>Data:</strong> ${dados.data_movimentacao ? new Date(dados.data_movimentacao).toLocaleString('pt-BR') : 'N/A'}</li>
                </ul>
            `;
        }
    } catch (e) {
        dadosDetalhados = '<p>Erro ao processar dados detalhados</p>';
    }

    modalContent.innerHTML = `
        <h3 style="color: #374151; margin-bottom: 1rem;">📊 Detalhes da Exclusão #${log.id}</h3>
        
        <div style="display: grid; gap: 1rem; margin-bottom: 1.5rem;">
            <div style="background: #f8fafc; padding: 1rem; border-radius: 6px;">
                <h5 style="margin: 0 0 0.5rem 0; color: #374151;">👤 Informações Gerais</h5>
                <p><strong>Tipo:</strong> ${log.tipo_exclusao === 'aih_completa' ? '💀 AIH Completa' : '🗑️ Movimentação'}</p>
                <p><strong>Usuário:</strong> ${log.usuario_nome || 'Sistema'}</p>
                <p><strong>Data/Hora:</strong> ${new Date(log.data_exclusao).toLocaleString('pt-BR')}</p>
                <p><strong>IP:</strong> ${log.ip_origem || 'N/A'}</p>
            </div>
            
            <div style="background: #fef3c7; padding: 1rem; border-radius: 6px; border-left: 4px solid #f59e0b;">
                <h5 style="margin: 0 0 0.5rem 0; color: #92400e;">📝 Justificativa</h5>
                <p style="margin: 0; font-style: italic;">"${log.justificativa || 'Nenhuma justificativa fornecida'}"</p>
            </div>
            
            <div style="background: #f0f9ff; padding: 1rem; border-radius: 6px; border-left: 4px solid #6366f1;">
                ${dadosDetalhados}
            </div>
        </div>
        ...
        <div style="text-align: center;">
            <button onclick="document.getElementById('modal').classList.remove('ativo')" 
                    style="background: #6366f1; color: white; border: none; padding: 0.75rem 2rem; border-radius: 6px; cursor: pointer; font-weight: 600;">
                ✅ Fechar
            </button>
        </div>
    `;
...
// Função para exportar logs de exclusão
window.exportarLogsExclusao = async () => {
    try {
        ...
        const response = await fetch('/api/relatorios/logs-exclusao/export', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${state.token}`
        ...
        if (!response.ok) {
            throw new Error('Erro ao exportar logs de exclusão');
        }
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `logs-exclusao-${new Date().toISOString().split('T')[0]}.xls`;
        link.click();
        URL.revokeObjectURL(url);

        alert('✅ Logs de exclusão exportados com sucesso!');
    } catch (err) {
        alert('❌ Erro ao exportar logs: ' + err.message);
    } finally {
        if (event.target) {
            event.target.textContent = '📊 Exportar';
            event.target.disabled = false;
        }
    }
